  ...
  return `
    <article class="event-card" id="${i.id}">
      ...
      <div class="source-row">
        ...
        <button class="details-btn" data-details="${i.id}">
          Ver →
        </button>
      </div>
...
function renderTimeline() {
  ...
  timeline.innerHTML =
    `<div class="day-node">07 AGO 2026 · VERIFICADO ✓</div>` +
    groups
      .map(
        ([k, l]) => `
  ...
  document.querySelectorAll(".details-btn").forEach((b) =>
    b.addEventListener("click", () => {
      const p = document.getElementById(`details-${b.dataset.details}`);
...
function doSearch(q) {
  ...
  const gh = Object.entries(glossary).filter(([t, d]) =>
  ...
  searchResults.innerHTML =
    hits
      .map(
        (e) => `
          <button class="search-hit" data-jump="${e.id}">
            <strong>${e.title}</strong>
            <span>${e.groupLabel} · ${e.category}</span>
          </button>
        `
      )
      .join("") +
...
document
  ...
  .addEventListener("click", () => searchDialog.showModal());
...
document
  ...
  .addEventListener("click", () => dateDialog.showModal());
...
const brebKey = document.getElementById("brebKey");
...
copyKeyBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    copyStatus.textContent = "Llave copiada ✓";
  ...
  setTimeout(() => {
    ...
    copyKeyBtn.textContent = "Copiar llave";
  }, 2200);
});
...
document.querySelectorAll(".nav-item").forEach((b) =>
  b.addEventListener("click", () => {
    ...
    if (["hoy", "dia"].includes(b.dataset.section)) {
      window.scrollTo({
        top: 0,
        behavior: "smooth",
      });
    ...
    if (b.dataset.section === "entiende") {
      searchDialog.showModal();

      searchInput.value = "presunción de inocencia";
